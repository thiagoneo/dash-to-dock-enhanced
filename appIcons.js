...
const DockAbstractAppIcon = GObject.registerClass({
    ...
    _init(app, monitorIndex, iconAnimator) {
        ...
        this._hoverPreviewTimeoutId = 0;
        this.connect('notify::hover', () => this._onHoverChangedForPreview());
    ...
    //
    // Root cause analysis:
    // 1. blockSourceEvents=true makes PopupMenuManager swallow icon enter/leave,
    //    so notify::hover never fires false while popup is open.
    // 2. _updateDashVisibility() in docking.js can slide the dock off-screen even
    //    while a menu is open (intellihide overlap), moving the icon off-screen.
    //    Any check against the icon's transformed position then returns false
    //    and wrongly closes the popup.
    //
    // Solution: poll the popup's bounding box ONLY (popup lives in Main.uiGroup
    // and never moves with the dock). Close only after N consecutive polls where
    // the cursor is outside popup + a generous margin (covers the BoxPointer gap).

    _onHoverChangedForPreview() {
        const settings = Docking.DockManager.settings;

        if (!settings.hoverWindowPreview) {
            this._cancelHoverPreview();
            return;
        }

        if (this.hover) {
            // Don't stack open timers; don't re-open if already open.
            if (this._hoverPreviewTimeoutId || this._previewMenu?.isOpen)
                return;

            const delay = settings.hoverPreviewDelay ?? 500;
            this._hoverPreviewTimeoutId = GLib.timeout_add(
                GLib.PRIORITY_DEFAULT, delay, () => {
                    this._hoverPreviewTimeoutId = 0;
                    if (this.hover &&
                        !this._previewMenu?.isOpen &&
                        this.getInterestingWindows().length > 0) {
                        this._windowPreviews();
                        this._startPreviewClosePoll();
                    }
    ...
    // Poll every 150 ms. Keep the popup open while the cursor is inside
    // EITHER the popup bounding box OR the icon bounding box (+ MARGIN).
    // We require THRESHOLD consecutive "outside" polls before closing, to
    // absorb the brief transit gap between icon and popup.
    _startPreviewClosePoll() {
        if (this._previewClosePollId) return;
        const MARGIN = 20;    // px around each actor
        const THRESHOLD = 2;  // consecutive outside-polls before closing
        let outsideCount = 0;

        this._previewClosePollId = GLib.timeout_add(
            GLib.PRIORITY_DEFAULT, 150, () => {
                if (!this._previewMenu?.isOpen) {
                    this._previewClosePollId = 0;
                    return GLib.SOURCE_REMOVE;
                }

                const [px, py] = global.get_pointer();

                // Check popup (anchored in uiGroup, never moves)
                const popupActor = this._previewMenu.actor;
                if (popupActor?.visible && popupActor.mapped) {
                    const [ax, ay] = popupActor.get_transformed_position();
                    const [aw, ah] = popupActor.get_transformed_size();
                    if (px >= ax - MARGIN && px <= ax + aw + MARGIN &&
                ...
                // Check icon (moves with dock, but dock is still while menu open)
                if (this?.visible && this.mapped) {
                    const [ix, iy] = this.get_transformed_position();
                    const [iw, ih] = this.get_transformed_size();
                    if (px >= ix - MARGIN && px <= ix + iw + MARGIN &&
                        py >= iy - MARGIN && py <= iy + ih + MARGIN) {
                        outsideCount = 0;
                        return GLib.SOURCE_CONTINUE;
                    }
                }

                // Cursor outside both areas
                outsideCount++;
                if (outsideCount < THRESHOLD)
                    return GLib.SOURCE_CONTINUE;
    ...
    _stopPreviewClosePoll() {
        if (this._previewClosePollId) {
            GLib.source_remove(this._previewClosePollId);
            this._previewClosePollId = 0;
        }
    }

    _cancelHoverPreview() {
        if (this._hoverPreviewTimeoutId) {
            GLib.source_remove(this._hoverPreviewTimeoutId);
            this._hoverPreviewTimeoutId = 0;
        }
    }
